...
from utils import train_utils_multiclass
    ...
    with tf.variable_scope('decoder', reuse={'train': None, 'test': True}[phase]):
        ...
        if H['arch']['use_rezoom']:
            if H['arch']['rezoom_change_loss'] == 'center':
                ...
                iou = train_utils_multiclass.iou(train_utils_multiclass.to_x1y1x2y2(tf.reshape(pred_boxes, [-1, 4])),
                                      train_utils_multiclass.to_x1y1x2y2(tf.reshape(perm_truth, [-1, 4])))
...
def train(H, test_images):
    ...
    def my_loop( sess, enqueue_op, phase, gen):
        for d in gen:
    ...
    with tf.Session(config=config) as sess:     
        ...
        threads = []
        for phase in ['train', 'test']:
            gen = train_utils_multiclass.load_data_gen(H, phase, jitter=H['solver']['use_jitter'])
            # enqueue once manually to avoid thread start delay
            t= tf.train.threading.Thread(target=my_loop, args=( sess, enqueue_op, phase, gen))
...
def main():
    ...
    if len(H.get('exp_name', '')) == 0:
        H['exp_name'] = args.hypes.split('/')[-1].replace('.json', '')+'_multiclass'
